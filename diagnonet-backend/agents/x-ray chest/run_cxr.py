...
def analyze_case(case_data: dict) -> dict:
    ...
    # Include patient demographics in the result
    patient_info = {}
    if case_data.get('patient_name'):
        patient_info['patient_name'] = case_data['patient_name']
    if case_data.get('age'):
        patient_info['age'] = case_data['age']
    if case_data.get('height_cm'):
        patient_info['height_cm'] = case_data['height_cm']

    if patient_info:
        result['patient_demographics'] = patient_info
...
def main():
    ...
    ap.add_argument("--patient-name", help="Patient name")
    ap.add_argument("--age", type=int, help="Patient age in years")
    ap.add_argument("--height-cm", type=float, help="Patient height in centimeters")
    ap.add_argument("--height-ft", type=float, help="Patient height in feet (will convert to cm)")
    ap.add_argument("--height-in", type=float, help="Patient height in inches (will convert to cm)")
    ...
    # Process height conversion
    height_cm = None
    if args.height_cm:
        height_cm = args.height_cm
    elif args.height_ft or args.height_in:
        # Convert feet and inches to cm
        feet = args.height_ft or 0
        inches = args.height_in or 0
    ...
    # Display patient demographics if provided
    if case_data.get('patient_name') or case_data.get('age') or case_data.get('height_cm'):
        print()
        demo_parts = []
        if case_data.get('patient_name'):
            demo_parts.append(f"PATIENT: {case_data['patient_name']}")
        if case_data.get('age'):
            demo_parts.append(f"AGE: {case_data['age']} yrs")
        if case_data.get('height_cm'):
            demo_parts.append(f"HEIGHT: {case_data['height_cm']:.0f} cm")
