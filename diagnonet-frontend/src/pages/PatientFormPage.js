...
const PatientFormPage = ({ onSubmit, onReset, isProcessing, processingStatus }) => {
  const navigate = useNavigate();
  const [patientInfo, setPatientInfo] = useState({
    name: '',
    age: '',
    height: '',
    heightUnit: 'ft' // 'ft' for feet/inches, 'cm' for centimeters
  });
  ...
  const validateForm = () => {
    ...
    if (!patientInfo.name.trim()) errors.name = 'Name is required';
    if (!patientInfo.age || patientInfo.age < 1 || patientInfo.age > 120) errors.age = 'Valid age is required';
    ...
    if (!patientInfo.height) {
      ...
      } else if (patientInfo.heightUnit === 'ft') {
        // Height in feet (reasonable range: 2-8 feet)
        if (height < 2 || height > 8) {
          errors.height = 'Height must be between 2 and 8 feet';
        }
      } else if (patientInfo.heightUnit === 'cm') {
        // Height in centimeters (reasonable range: 60-250 cm)
        if (height < 60 || height > 250) {
          errors.height = 'Height must be between 60 and 250 cm';
        }
      }
    }

    // Validate selected agents
    if (selectedAgents.length === 0) errors.agents = 'Please select at least one diagnostic agent';

    // Validate agent-specific data
    selectedAgents.forEach(agent => {
      if (agent === 'vital') {
        const vital = agentData.vital;
        if (!vital.systolicBP || !vital.diastolicBP || !vital.heartRate || !vital.temperature) {
          errors.vital = 'All vital signs are required';
        }
      }
      if (agent === 'symptom') {
        if (!agentData.symptom.symptoms.trim()) {
          errors.symptom = 'Symptom description is required';
        }
      }
      if (agent === 'xray') {
        if (!agentData.xray.imageFile) {
          errors.xray = 'X-ray image is required';
        }
      }
    });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Navigate to results page only after processing is complete
      if (result && result.success) {
        ...
        // If submission failed, stay on form page
  ...
  if (isSubmitting || isProcessing) {
    return (
      <div className="loading">
        ...
        <p>{processingStatus || 'Our AI agents are analyzing your information'}</p>
        <div className="processing-agents">
          {selectedAgents.map(agent => (
            <div key={agent} className="agent-status">
              <span className="agent-name">{agent.charAt(0).toUpperCase() + agent.slice(1)} Agent</span>
            ...
            <div className="form-group">
              ...
              <div className="height-input-container">
                <input
                  ...
                  step="0.1"
                  min="0"
                  placeholder={patientInfo.heightUnit === 'ft' ? 'e.g., 5.8' : 'e.g., 175'}
                />
                <select
                  name="heightUnit"
                  value={patientInfo.heightUnit}
                  onChange={handlePatientInfoChange}
                  className="form-input height-unit-select"
                >
              ...
              <small className="form-help-text">
                {patientInfo.heightUnit === 'ft'
                  ? 'Enter height in feet (e.g., 5.8 for 5 feet 8 inches)'
                  : 'Enter height in centimeters (e.g., 175)'
                }
              </small>
              {validationErrors.height && <span className="error-message">{validationErrors.height}</span>}
            </div>
          </div>
